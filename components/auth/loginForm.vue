...
import {type FormItemRule, type FormRules, NAlert, NButton, NDivider, NForm, NFormItem, NInput} from 'naive-ui'
  ...
  <div class="tw-flex tw-flex-col tw-gap-4 tw-max-w-[500px] tw-w-full">
    ...
    <n-form ref="formRef" :model="formValues" :rules="rules" :size="'medium'">
      ...
      <n-form-item :show-label="false" path="password">
        ...
        >
          <template #prefix>
            <Icon color="var(--secondary)" name="solar:lock-keyhole-outline" size="20"/>
          </template>
        </n-input>
      </n-form-item>
      <h6 class="subtitle tw-text-end hover:tw-underline tw-cursor-pointer" @click="$emit('resetPass')">Forgot
        password?</h6>
    </n-form>
    <n-alert v-if="errorMsg" :show-icon="false" :title="errorMsg" closable type="error"/>
    <n-button :loading="loading" type="primary" @click="submit">
      Submit
    </n-button>
    <h6 class="tw-text-secondary tw-text-sm tw-text-center">don't have an account? <span
        class="tw-text-primary tw-font-semibold tw-underline tw-cursor-pointer">click here</span></h6>
